...
import com.dtakac.aux_remote.common.SERVER_QUEUE_LIST
...
class ResponseHandlerService: JobIntentService(){
    ...
    private fun onQueueList(queuedSongs: List<String>){
        val result = mutableListOf<QueuedSong>()
        for(i in queuedSongs.indices step 2){
            val name = queuedSongs[i]
            val ownerId = queuedSongs[i+1]
            val userIconVisibility = if(ownerId == prefsRepo.get(PREFS_USER_ID, "")) View.VISIBLE else View.GONE
            result.add(QueuedSong(ownerId, name, userIconVisibility))
        }
